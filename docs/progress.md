...
## Current Phase: Phase E — AI-Driven Flows (active). E1 (incl. router→chat wiring) + E2 ✓; E0a, E3, E4, E5, E7 remain.
...
- [ ] **E3. Confirmation surface (§6.11).** *Unblocked; a STUB page already exists at `extract/[sessionId]` (router `log` branch + uploads land there) — replace it.* Real split-panel: source left (text block / `FilePreview` via `mimeFromPath`), cards right; ambiguity inline-resolve chips (block Confirm); new-vs-update toggle; per-card Confirm/Discard/Edit; `Confirm all · N`; commit writes entities with `source_report_id` + report/session→`committed` + fires debounced insight-gen. NO floating Ask AI. **Also lands: the upload entry point UI, the drawer router-wiring, and the §6.2:1197 post-commit "inline Logged · continue" + §5.5 compound post-confirm synthesis-forward.**

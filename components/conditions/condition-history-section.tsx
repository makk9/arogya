...
/*
 * History section client island. Owns the `Show all` / `Hide` toggle. The
 * `+ Log a change` button reads its onClick from ConditionLogChangeContext.
 * Clones medication-history-section.tsx. Unlike Medication, the provider is
 * always present (Condition has no terminal state that hides the affordance),
 * so the button always renders.
 *
 * Per locked refinement, the Show-all affordance only appears when there are
 * more than 5 changes. Empty-history copy renders inline alongside the button.
 */

...
### Last Session
- 2026-06-17 — **Symptom vertical: live-test review (8 UX fixes) + `/check` hardening (5 eng fixes), committed.** Review fixes: form defaults to "+ Create new" (dropdown hidden at 0 types); episode Linked-context = **siblings only** (no self-link; parent moved to header chip); **post-hoc vital link/unlink in episode Edit** (`linkedVitalIds` now PATCH-able, scope-checked); **"+ Log episode" from a type locks** the symptom field + returns to that type page; episode `EPISODE` tag + terse "↑ View symptom →"; type "This symptom and every episode…" descriptor; timeline "View symptom →".
- **`/check` eng fixes:** **vital DELETE scrubs the id from episodes' `linked_vital_ids`** (was dangling → `§ unresolved:` noise, demo-relevant); dropped a `!` for an explicit guard; single-source `lib/vitals.ts` (`formatVitalValue`) + `lib/select-sentinel.ts` (`NOT_SET`) — both re-exported, **0 importer churn**; form rejects `duration < 1`; siblings deliberately unbounded (count needs the full set).
- **Verified:** `/verify-ui` **18✅ / 6🔍 / 0❌** (real headless browser — all 8 fixes + timeline/episode/type/form/by-slug/probes); scrub proven via API; `tsc`/`eslint`/`citation-parser` clean; data restored to baseline (ZZZ swept).

### Next Steps
1. **Report / JournalEntry** — remaining event entities (Report = Lab's read-only-body + extracted-outcomes; Journal = no Outcomes/Notes, §-pills, `(untitled)`). Then **Patient profile** (6.10) + **Insights** (6.8/6.9 placeholder) parallelize.
2. When the insight/extraction layers land, run the two **Phase-E audits** queued in decisions.md (2026-06-17): vital chat-quick-log entry path; symptom↔condition duplicate reconciliation via the insight layer.

...
### Last Session
- 2026-06-09 — **Condition vertical COMPLETE** — detail page (6.5) + inline-edit + `+ Log a change` + Delete + live `§ condition:` pill. `app/patient/[id]/conditions/[conditionId]/page.tsx` + 13 `components/conditions/condition-*` files, cloned off the Med detail tree. **Divergences from Med:** no status-freezing (Condition has no terminal state — resolved→active is valid); neutral tokenized status pill (did NOT copy med header's hardcoded emerald/amber); `…`=Delete only (FK-safe: `med.purpose`+`lab.linkedCondition` both onDelete:set-null); +Log a change = status/severity/managing_doctor, NO linkedVisitId; every `<Select>` passes `items`.
- **TDZ root-cause fixed (cross-cutting):** `condition-options.ts`+`medication-options.ts` value-imported `@/db/schema` but used the enums only in `typeof` positions → that runtime edge dragged the DB layer into the client bundle and crashed `citation-pill` (loaded early via chat-drawer-provider) with `STATUS_OPTIONS is not defined`. Fix: both → `import type`. **New convention: entity `*-options.ts` import schema enums as `import type`.**
...
- **Pending doc-fixes (next design.md edit):** (§6.5) permit Notes omission when empty + broaden "Edit toggles Current fields" to all non-clinical (clinical → +Log a change); (§6.2/§6.4) floating Ask AI = context-preserving push drawer (non-modal), NOT full-screen — rail "Chat" + chat persistence remain Phase D (no chat-session table v1).
...
- **Citation-pill limitations (v1.5):** collision-suffixed citations (`med:x-2`/`condition:x-2`) resolve to null → misleading "not in record"; med popover omits **prescribing doctor** (the demo's cross-doctor field) — add when the Doctor picker/join lands.
- **Phase D carryovers (non-blocking):** **med detail header hardcodes emerald/amber status-pill colors** (latent 7.2 violation — fix when next touching med surfaces); **Base UI `items` bug** still on **med form + med filter** (Condition surfaces all fixed); **E2** extract shared `<EntityList>`/`Section` at Doctor; **D2** inert entity refs (Condition's managing/diagnosing doctor; `diagnosedBy` unsettable via UI) become links/editable once Doctor lands. Older: enum-validation → `lib/schemas/url-filters.ts`; `useId()` panel IDs; drawer `32rem` magic-number; `chat-drawer-provider` does 4 jobs; **periwinkle dark-mode (v1.5)** no `.dark`; **SDK** `@ai-sdk/anthropic` 3.0.x missing top-level `cacheReadInputTokens`.

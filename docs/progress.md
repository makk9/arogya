...
- **Pending doc-fixes (apply alongside next design.md edit):** (§6.5) permit Notes omission when empty + broaden "Edit toggles Current-section fields" to all non-clinical fields (clinical route through + Log a change); (§6.2/§6.4) floating Ask AI opens a **context-preserving push drawer** (non-modal, no backdrop), NOT full-screen nav — the three-column rail "Chat" surface + chat persistence-across-refresh remain Phase D (no chat-session table in v1 schema).
...
- **Drawer close-animation sync:** padding releases as drawer slides out (both 300ms) — visually fine; tighten if a seam shows. Drawer width fixed at 32rem (`sm:max-w-lg` + matching `sm:pr-[32rem]`) — tunable if cramped on narrow desktops.
